...
<template>
  <div>
    <Card :bordered="false" shadow class="info-card-wrapper">
      <p slot="title">
        成员管理
        <span class="info">被添加到项目成员后每天都会收到灯塔质量报告</span>
        <AddUser v-on:listenAdd="addNew"></AddUser>
      </p>
      <Table ref="selection" :columns="columns" :data="list"></Table>
    </Card>
    <Modal v-model="isEdit" title="成员角色编辑" @on-ok="saveEdit" @on-cancel="cancelEdit">
      <Form :model="editParam" label-position="left" :label-width="100">
        <FormItem label="账号">
          <Input style="width:300px" disabled :value="editIndex.ucid || ''" />
        </FormItem>
        <FormItem label="名字">
          <Input style="width:300px" disabled :value="editIndex.nickname || ''" />
        </FormItem>
        <FormItem label="角色">
...
<script>
  ...
  import { getMemberDel, getMemberInfo, getMemberUpdate } from 'src/api/member'
  ...
  export default {
    ...
    methods: {
      ...
      // 删除用户数据
      async getMemberDel(param) {
        const res = await getMemberDel(param)
        this.delMsg = res.msg
      },
      // 编辑某一行，弹窗显示并传值
      edit(param) {
