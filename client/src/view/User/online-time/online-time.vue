...
export default {
  ...
  data () {
    return {
      ...
      condition: {
        dateRange: [moment().startOf('day').toDate(), moment().toDate()],
        filterBy: 'hour'
      }
    }
  },
  methods: {
    ...
    async getLineData () {
      ...
      const res = await BehaviorApi.getOnlineTime(
        this.condition.filterBy,
        +moment(this.condition.dateRange[0]),
        +moment(this.condition.dateRange[1])
      )
