...
export const get = (object, path, defaultValue) => {
  let _path = getTag(path) === '[object Array]' ? path : String.prototype.split.call(path, /[,[\].]+?/).filter(Boolean)
  let index = 0
...
export function getDeviceId () {
  ...
  if (deviceId === undefined) {
    ...
    Cookies.set(COOKIE_NAME_DIVICE_ID, deviceId, { expires: 1000, domain: domain })
...
export function customerErrorCheck (commonConfig, desc, stack, debugLogger = console.log) {

  // 调用自定义函数, 检测是否需要上报错误
  let customerErrorCheckFunc = _.get(commonConfig, ['record', 'js_error_report_config', 'checkErrorNeedReport'],  _.get(DEFAULT_CONFIG, ['record', 'js_error_report_config', 'checkErrorNeedReport']))
  let isNeedReport = false

  try {
    isNeedReport = customerErrorCheckFunc(desc, stack)
  } catch (e) {
    debugLogger(`config.record.js_error_report_config.checkErrorNeedReport执行时发生异常, 请注意, 页面报错信息为=>`, { e, desc, stack })
    isNeedReport = true
  }
...
export const validLog = (commonConfig, type = '', code, detail = {}, extra = {}) => {
